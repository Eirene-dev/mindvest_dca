...
STATE_STRATEGY_MATRIX = {
    'S0': {  # Market Crash
        ...
        'max_amount_multiplier': 0.0,  # 신규 추가
        'interval_hours': 24,
        'tp_ratio': 5,
        'sl_ratio': -3,
        'allowed_symbols': [],
    ...
    'S3': {  # Consolidation Low
        ...
        'max_amount_multiplier': 0.3,  # 최대 30%만 허용
        'interval_hours': 12,
        'tp_ratio': 3,
        'sl_ratio': -7,
    ...
    'S4': {  # Neutral
        ...
        'max_amount_multiplier': 0.5,  # 최대 50%
        'interval_hours': 8,
        'tp_ratio': 5,
        'sl_ratio': -10,
    ...
    'S5': {  # Early Bull
        ...
        'max_amount_multiplier': 0.7,  # 최대 70%
        'interval_hours': 6,
        'tp_ratio': 7,
        'sl_ratio': -10,
    ...
    'S6': {  # Strong Bull
        ...
        'max_amount_multiplier': 1.0,  # 최대 100%
        'interval_hours': 4,
        'tp_ratio': 10,
        'sl_ratio': -12,
        'allowed_symbols': ['BTC', 'ETH', 'SOL', 'LINK', 'XRP', 'ADA', 'DOGE'],
    ...
    'S7': {  # Overheated
        ...
        'max_amount_multiplier': 0.6,  # 과열시 60%로 축소
        'interval_hours': 4,
        'tp_ratio': 5,
        'sl_ratio': -8,
    ...
    'S8': {  # Distribution
        ...
        'max_amount_multiplier': 0.3,  # 분산시 30%만
        'interval_hours': 8,
        'tp_ratio': 3,
        'sl_ratio': -5,
        'allowed_symbols': ['BTC', 'ETH'],
...
POSITION_ADJUSTMENT_MATRIX = {
    ...
    # 기본값 (정의되지 않은 조합)
    'DEFAULT': {'amount_mult': 0.5, 'max_mult': 0.5, 'tp_mult': 1.0, 'sl_mult': 1.0}
}
...
# async def position_analysis(update: Update, context: ContextTypes.DEFAULT_TYPE):
#     """자산 위치 분석 리포트"""
#     global trader
#     report = await trader.get_position_analysis_report()
...
#     output = f"""
# Current Market State: {state_manager.current_state}
# Description: {state_params['description']}
# Confidence: {state_manager.state_confidence:.1%}
# Last Update: {state_manager.last_update_time.strftime('%Y-%m-%d %H:%M:%S')}
#     """
#     await update.message.reply_text(output)

# async def state_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
#     """시장 상태 히스토리"""
#     history = db_manager.get_state_history(24)
...
#     """리스크 관리 리포트"""
#     global trader
#     report = await trader.get_risk_report()
#     await update.message.reply_text(report)
...
#     for symbol, pos_info in trader.pos_info_dict.items():
#         pos_info.update(BinanceTrader.client)
...
#         # 현재 포지션
#         pos_info = trader.pos_info_dict[symbol]
#         pos = pos_info.position_amt['LONG']
#         if pos > 0:
...
#         if TradeConfig.current_symbol:
#             cfg = TradeConfig.trade_config[TradeConfig.current_symbol]
#             output = f"**Current Symbol: {TradeConfig.current_symbol}**\n"
...
#         if symbol in TradeConfig.trade_config:
...
#     if len(context.args) == 0:
#         output = f"Trading Volume for {TradeConfig.current_symbol}: ${symbol_config.open_amount:.0f}"
#     elif len(context.args) == 1:
#         try:
#             vol = float(context.args[0])
#             symbol_config.base_open_amount = vol
#             symbol_config.open_amount = vol
#             output = f"Volume set to ${vol:.0f} for {TradeConfig.current_symbol}"
#         except ValueError:
...
# async def max_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
#     """최대 거래 한도 설정"""
#     symbol_config = TradeConfig.current_symbol_config()
#     if not symbol_config:
#         await update.message.reply_text("No symbol selected. Use /current SYMBOL first")
...
#     if len(context.args) == 0:
#         output = f"Max Amount for {TradeConfig.current_symbol}: ${symbol_config.max_amount:.0f}"
#     elif len(context.args) == 1:
#         try:
#             max_amt = float(context.args[0])
#             symbol_config.base_max_amount = max_amt
#             symbol_config.max_amount = max_amt
#             output = f"Max amount set to ${max_amt:.0f} for {TradeConfig.current_symbol}"
#         except ValueError:
...
#     if len(context.args) == 0:
#         output = f"TP Ratio for {TradeConfig.current_symbol}: {symbol_config.take_profit_ratio}%"
#     elif len(context.args) == 1:
#         try:
#             ratio = float(context.args[0])
#             symbol_config.base_tp_ratio = ratio
#             symbol_config.take_profit_ratio = ratio
#             output = f"TP ratio set to {ratio}% for {TradeConfig.current_symbol}"
#         except ValueError:
...
# async def stop_loss_ratio(update: Update, context: ContextTypes.DEFAULT_TYPE):
#     """손절 비율 설정"""
#     symbol_config = TradeConfig.current_symbol_config()
#     if not symbol_config:
#         await update.message.reply_text("No symbol selected. Use /current SYMBOL first")
...
#     if len(context.args) == 0:
#         output = f"SL Ratio for {TradeConfig.current_symbol}: {symbol_config.stop_loss_ratio}%"
#     elif len(context.args) == 1:
#         try:
#             ratio = float(context.args[0])
#             symbol_config.base_sl_ratio = ratio
#             symbol_config.stop_loss_ratio = ratio
#             output = f"SL ratio set to {ratio}% for {TradeConfig.current_symbol}"
#         except ValueError:
...
#     """모든 자동매매 일시정지"""
#     for cfg in TradeConfig.trade_config.values():
#         cfg.stop_trade = True
    
#     output = "⛔ **All Trading Stopped**\n"
#     output += "Use /start_all to resume trading"
#     await update.message.reply_text(output)
...
#     output += "Trading will continue based on market state"
#     await update.message.reply_text(output)
...
#     if len(context.args) == 0:
#         output = f"Stop Trade for {TradeConfig.current_symbol}: {'Yes' if symbol_config.stop_trade else 'No'}"
#     elif len(context.args) == 1:
#         mode = str(context.args[0]).lower()
#         if mode in ['on', 'yes', '1', 'true']:
#             symbol_config.stop_trade = True
#             output = f"Trading stopped for {TradeConfig.current_symbol}"
#         elif mode in ['off', 'no', '0', 'false']:
#             symbol_config.stop_trade = False
#             output = f"Trading resumed for {TradeConfig.current_symbol}"
#         else:
...
# async def reduce_position(update: Update, context: ContextTypes.DEFAULT_TYPE):
#     """포지션 부분 청산"""
...
#         symbol = str(context.args[0]).upper()
#         try:
#             percentage = float(context.args[1]) / 100
#             if symbol in trader.pos_info_dict:
#                 await trader.reduce_position(symbol, percentage)
...
#     output += "━━━━━━━━━━━━━━━━━━━━\n"
#     output += f"🔹 Market State: {state_manager.current_state}\n"
#     output += f"🔹 Description: {state_params['description']}\n"
#     output += f"🔹 Confidence: {state_manager.state_confidence:.1%}\n"
#     output += f"🔹 Active Symbols: {', '.join(state_params['allowed_symbols'])}\n"
#     output += f"\n**Trading Status**\n"
...
# async def guide_short(update: Update, context: ContextTypes.DEFAULT_TYPE):
#     """간단한 지표 해석 가이드"""
...
if __name__ == "__main__":
    ...
    # application.add_handler(CommandHandler("max_amount", max_amount))
    # application.add_handler(CommandHandler("take_profit_ratio", take_profit_ratio))
    # application.add_handler(CommandHandler("stop_loss_ratio", stop_loss_ratio))
    # application.add_handler(CommandHandler("stop_all", stop_all))
    # application.add_handler(CommandHandler("start_all", start_all))
    # application.add_handler(CommandHandler("stop_trade", stop_trade))
    # application.add_handler(CommandHandler("close_position", close_position))
    # application.add_handler(CommandHandler("reduce_position", reduce_position))
    # application.add_handler(CommandHandler("market_state", market_state))
    # application.add_handler(CommandHandler("position_analysis", position_analysis))
    # application.add_handler(CommandHandler("risk_report", risk_report))
    # application.add_handler(CommandHandler("state_history", state_history))
    ...
    # application.add_handler(CommandHandler("guide_short", guide_short))
